...
t_arr = np.linspace(0, t_run, int(t_run/0.001))*second
raster_trials_plot = [0, 0, 0]
filename = 'RESULTS'
# filename= 'Diff_mn_Results_N=1000, J =0.5 mV, N_trials=100, tau_short=10. ms, tau_long=100. ms'
if generate_res:
    ...
    spikes_res_orth = SPIKES_C(Sm = Sm, Sn = Sn, J_value = J_syn, factor_mn = factor_mn, I_ampl =  I_o, ind_c = 0, c_array = c_arr, mu_noise = mu_noise*mV,
                    ...
                    tau_s= tau_long, tau_min = tau_D_min, tau_max = tau_D_max, inhib_g = g, seed_start=2, t_run =t_run)
    ...
    spikes_res_equal = SPIKES_C(Sm = Sm, Sn = Sn, J_value = J_syn, factor_mn = factor_mn, I_ampl = I_e, ind_c = 0, c_array = c_arr, mu_noise = mu_noise*mV,
                    ...
                    tau_s= tau_long, tau_min = tau_D_min, tau_max = tau_D_max, inhib_g = g, seed_start=3, t_run = t_run)
...
for tau in tau_all:
    ...
    if tau == 1*ms:
        ...
        print('min 1 = '+str(np.min(IM1)))
        print('max 1 = '+str(np.max(IM1)))
        plt.clim([0,1])
    ...
    if tau == 100*ms:
        ...
        PCA_projs = np.absolute(np.asarray(PCA_projs_a))
        IM1 = PCA_projs.transpose()
        print('min 100 = '+str(np.min(IM1)))
        print('max 100 = '+str(np.max(IM1)))
